...
function rewardsToStr(rewards) {
  ...
  return Object.entries(rewards).map(([k, v]) => `${k}+${v}`).join(', ');
}
...
Page({
    ...
    wx.cloud.callFunction({
      ...
      success: (res) => {
        ...
        if (result.ok) {
          if (result.failed) {
            wx.showModal({
              title: `突破失败！${result.fromRealm} → ${result.toRealm}`,
              content: result.message,
              showCancel: false,
              confirmText: '继续修行',
              success: () => { this.loadPanel(); },
            });
          } else if (result.isMajor) {
            wx.showModal({
              title: `突破成功！${result.fromRealm} → ${result.toRealm}`,
              content: result.message,
              showCancel: false,
              confirmText: '继续修行',
              success: () => { this.loadPanel(); },
  ...
  onNewReminderTimeChange(e) {
    ...
    this.setData({ newReminderTime: time });
    this._addReminder(time);
  },
  ...
  _addReminder(time) {
    ...
    wx.cloud.callFunction({
      ...
      success: (res) => {
        ...
        if (result.ok) {
          wx.showToast({ title: result.message, icon: 'success' });
          this.setData({ newReminderContent: '' });
          this.loadPanel();
        } else {
  ...
  authorizeReminder() {
    wx.requestSubscribeMessage({
      ...
      fail: (err) => {
        console.error('订阅消息授权失败：', JSON.stringify(err));
        if (err.errCode === 20004) {
          wx.showModal({
            title: '推送通知未开启',
            content: '你之前拒绝了通知授权。请到小程序设置中手动开启「订阅消息」。',
            confirmText: '去设置',
            success: (res) => {
  ...
  // ========== 每日运势 ==========
  _checkDailyFortune() {
    const lastDate = wx.getStorageSync('last_fortune_date');
    const today = this._getTodayDate();
    const bazi = wx.getStorageSync('bazi');
    const panelGuideCompleted = wx.getStorageSync('panel_guide_completed');
    if (!bazi || !panelGuideCompleted) return; // 未填出生信息或引导未完成不弹

    if (lastDate !== today) {
      // 调用云函数获取简版运势
      wx.cloud.callFunction({
        name: 'dailyFortune',
        data: { birth_bazi: bazi, target_date: today },
        success: (res) => {
          if (res.result && res.result.ok) {
            ...
            wx.setStorageSync('last_fortune_date', today);
            wx.setStorageSync('cached_fortune', res.result);
          }
        },
      });
    } else {
      // 今天已看过，显示收纳状态
      const cachedFortune = wx.getStorageSync('cached_fortune');
      if (cachedFortune) {
        this.setData({ fortuneData: cachedFortune, fortuneMinimized: true });
      }
    }
  },

  closeFortuneModal() {
    this.setData({ showFortuneModal: false, fortuneMinimized: true });
    // 缓存运势数据
    if (this.data.fortuneData) {
      wx.setStorageSync('cached_fortune', this.data.fortuneData);
    }
  },
