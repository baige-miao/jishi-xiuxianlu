...
exports.main = async (event, context) => {
  ...
  try {
    switch (action) {
      ...
      case 'listMood':
        return await handleListMood(openid, event);
      default:
        return { ok: false, message: '未知操作：' + action };
    }
  } catch (err) {
    console.error(JSON.stringify({ func: 'diary', openid, error: err.message, stack: err.stack }));
...
// 获取指定月份的心情数据（轻量查询）
async function handleListMood(openid, event) {
  const { year, month } = event;
  if (!year || !month) return { ok: false, message: '缺少年月参数' };
  ...
  const res = await db.collection('diaries')
    ...
    .field({ date: true, mood_emoji: true })
    .orderBy('date', 'asc')
    .get();

  return { ok: true, moods: res.data };
}
