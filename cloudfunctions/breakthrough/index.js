  ...
  try {
    ...
    if (isPreview) {
      ...
      if (info.isMajor && baseFailRate > 0 && boosts.length > 0) {
        ...
        try {
          ...
          playerData.totalTasks = tasks.filter(t => t.completed || t.status === 'done').length;
          playerData.stageTasks = tasks.filter(t => t.type === '阶段' && (t.completed || t.status === 'done')).length;
        } catch (e) {}
        playerData.skillAttrCount = attrs.filter(a => a.grp === '学识' || a.grp === '技能').length;
        // 查询日记
        try {
          const diaryRes = await db.collection('diary').where({ _openid: openid }).count();
    ...
    if (info.isMajor && baseFailRate > 0 && boosts.length > 0) {
      ...
      try {
        const tasksRes = await db.collection('tasks').where({ _openid: openid }).get();
        const tasks = tasksRes.data || [];
        totalTasks = tasks.filter(t => t.completed || t.status === 'done').length;
        stageTasks = tasks.filter(t => t.type === '阶段' && (t.completed || t.status === 'done')).length;
      } catch (e) { /* 集合可能不存在 */ }
    ...
    console.error('修仙突破失败：', err);
    return { ok: false, message: '突破失败：' + err.message };
  }
};
