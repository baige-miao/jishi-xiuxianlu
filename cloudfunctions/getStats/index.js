...
const { getUserRating, MIND_ATTRS } = require('./levels');
...
exports.main = async (event, context) => {
  ...
  const weekStart = new Date(now.getTime() - (dayOfWeek - 1) * 24 * 60 * 60 * 1000).toISOString().slice(0, 10);
  // 本月起始
  ...
  try {
    ...
    // --- 4. 灵力累计获取量 ---
    const totalSpiritEarned = player.spirit_stone || 0;
    ...
    return { ok: false, message: '获取统计失败：' + err.message };
  }
};
