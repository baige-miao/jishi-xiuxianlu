...
// 属性分组 → 专属等级表映射（当属性名不在 CUSTOM_LEVEL_MAP 中时，按分组匹配）
const GROUP_LEVEL_MAP = {
  '体质': BODY_LEVELS,
  '心境': MENTAL_LEVELS,
  '学识': KNOWLEDGE_LEVELS,
};
...
/**
 * 根据属性名、分组和数值返回等级描述
 * 优先按属性名匹配专属等级，其次按分组匹配，最后使用通用等级
 * @param {string} attrName - 属性名
 * @param {number} value - 属性值
 * @param {string} [grp] - 属性分组（可选）
 * @returns {string} 等级描述
 */
function getLevel(attrName, value, grp) {
  const table = CUSTOM_LEVEL_MAP[attrName] || (grp && GROUP_LEVEL_MAP[grp]) || STD_LEVELS;
